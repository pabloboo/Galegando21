...
## OBTENCIÓN DE PALABRAS COMUNES EN INGLÉS
directorio_actual = os.getcwd()
print("Directorio actual:", directorio_actual)
ruta_archivo = './english_cambridge_pet.txt'
# Función para obtener las palabras comunes en inglés
...
for texto in textos:
    ...
    try:
        ...
        select_element = WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.ID, "idioma_orixe"))
        )
        select = Select(select_element)
        select.select_by_value('42203')  # Seleccionar el valor correspondiente al inglés

        # Esperar y hacer clic en el botón "Aceptar" del mensaje emergente
        aceptar_button = WebDriverWait(driver, 20).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "div.message input.close.default[value='Aceptar']"))
        )
        ...
        with open("palabras_comuns_gl_pet.json", "a", encoding="utf-8") as json_file:
                json.dump({"palabra": palabra}, json_file, ensure_ascii=False, indent=4)
...
with open("palabras_comuns_gl_pet_completo.json", "w", encoding="utf-8") as json_file:
